...
app = Flask(__name__)
model = load_model('latest.h5',compile=True)
...
def upload_image():
    ...
    if file and allowed_file(file.filename):
        ...
        test_img  = img.resize((IMAGE_WIDTH, IMAGE_HEIGHT))
        test_img = image.img_to_array(test_img)
        test_img = np.expand_dims(test_img, axis=0)
        # x  = image.img_to_array(img)
        # x = np.expand_dims(x, axis=0)
        # x  = preprocess_input(x)
        cl = model.predict(test_img)
        ...
        if cl[0][0]==1:
            return render_template('ImageML.html', prediction = 'Covid')
        else:
            return render_template('ImageML.html', prediction = 'Normal')
